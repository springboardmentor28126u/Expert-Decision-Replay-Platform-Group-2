...
class UserService:
    ...
    def __init__(self, db: Session):
        # Store database session so service methods can use self.db
        self.db = db

        # Existing repository and audit service
    ...
    def get_reviewers(self) -> List[User]:
        """Get all users whose role is Reviewer."""
    ...
    def update_user(
        ...
        if data.email and data.email != user.email:
            if self.user_repo.email_exists(
                data.email,
                exclude_id=user_id,
            ):
    ...
    def change_role(
        ...
        logger.info(
            f"User {user_id} role changed to {role}"
        )
