import { getReviewerDashboard } from "./api/dashboardService";
...
/**
 * ReviewerDashboard - Reviewer role dashboard page content.
 *
 * Props:
 ...
 *
 * Fetches GET /dashboard/reviewer via the Dashboard API service and renders:
 *   - Loading spinner while the initial request is in flight
 *   - Error panel with retry if the API call fails
 *   - 3 responsive StatCards (Under Review / Approved / Rejected)
 *   - RecentDecisionTable with the recent under-review decisions list
 */
export default function ReviewerDashboard({ token, onSelectDecision = null }) {
  ...
  const stats = data || {};
  const recentUnderReview = data?.recent_under_review || [];
  ...
  return (
    ...
    >
      ...
      {!showError && (
        <>
          {/* Top section: 3 responsive StatCards */}
          <div
            style={{
              display: "grid",
          ...
          {/* Bottom section: Recent under-review decisions */}
